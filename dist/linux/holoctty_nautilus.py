...
DOMAIN = "com.sfire.holoctty"
locale_dir = Path(__file__).absolute().parents[2] / "locale"
_ = gettext.translation(DOMAIN, locale_dir, fallback=True).gettext

def open_in_holoctty_activated(_menu, paths):
    for path in paths:
        cmd = ['holoctty', f'--working-directory={path}', '--gtk-single-instance=false']
...
def get_items_for_files(name, files):
    ...
    if paths:
        item = Nautilus.MenuItem(name=name, label=_('Open in Holoctty'),
            icon='com.sfire.holoctty')
        item.connect('activate', open_in_holoctty_activated, paths)
        return [item]
    else:
        return []


class HolocttyMenuProvider(GObject.GObject, Nautilus.MenuProvider):
    def get_file_items(self, files):
        return get_items_for_files('HolocttyNautilus::open_in_holoctty', files)

    def get_background_items(self, file):
        return get_items_for_files('HolocttyNautilus::open_folder_in_holoctty', [file])
