# Localizing Holoctty: The Translators' Guide

First of all, thanks for helping us localize Holoctty!
...
Full locale names are more complicated, but Holoctty does not use all parts. [The
...
file called `com.sfire.holoctty.pot`. **Do not edit this file.** The
template is generated automatically from Holoctty's code and resources, and is
...
```console
$ msginit -i po/com.sfire.holoctty.pot -l X -o "po/X.po"
```
...
Afterwards, you need to update the list of known locales within Holoctty's
...
Simply run `zig build run`. Holoctty uses your system language by default; if
...
On some desktop environments, such as KDE Plasma, Holoctty uses server-side
decorations by default. This hides many strings from the UI, which is
undesirable when viewing your translations. You can force Holoctty to use
...
help with Holoctty translations.

### `CODEOWNERS`

Localization teams are represented as teams in the Holoctty GitHub organization.
...
```diff
 ...
 /po/com.sfire.holoctty.pot @ghostty-org/localization
...
- **Make Holoctty fit in with surrounding applications.**
  ...
  interface guidelines of each platform Holoctty is available for, including the
